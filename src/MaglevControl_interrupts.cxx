...
MaglevControl::maglevStartForceController()
{
	...
	double interpolation_constant = 0.0;
	while(controller_counter <= NUM_TRANSITIONS)
	{
	        if (gains_changed)
	        {
	                if ((double)controller_counter / NUM_TRANSITIONS >= interpolation_constant + 9.9/NUM_TRANSITIONS)
	                {
	                        gains_changed = false;
	                }
	        }
	        ...
	        {
	                interpolation_constant = (double)controller_counter / NUM_TRANSITIONS;
		        ...
		        gains_changed = true;
        		//printf("\t\t%03d | %03d | %5.2f | i%7.2f | p%7.2f | f%5.2f\n", controller_counter, save_force_counter, interpolation_constant, current_internal_gains[0][0], current_position_gains[0][0], current_force_gains[0][0]);
	        }
	}
	//maglevPrintGainMatrices();
...
MaglevControl::maglevStopForceController()
{
	...
	double interpolation_constant = 0.0;
	static int start_counter = controller_counter;
        double starting_internal_gains[6][4];
        double starting_position_gains[6][4];
        double starting_force_gains[6][3];
        bool gains_changed = false;
	...
	while((controller_counter - start_counter) <= NUM_TRANSITIONS)
	{
	        if (gains_changed)
	        {
	                if ((double)controller_counter / NUM_TRANSITIONS >= interpolation_constant + 9.9/NUM_TRANSITIONS)
	        ...
	        {
	                interpolation_constant = (double)(controller_counter - start_counter) / NUM_TRANSITIONS;
		        for(int i=FORCE_START_INDEX; i<FORCE_STOP_INDEX; i++)
		        {
			        for(int j=0; j<3; j++)
			        {
		        ...
		        maglevSetInternalGains();
		        //printf("%03d | %7.2f\n", controller_counter, current_internal_gains[0][0]);
		        gains_changed = true;
	        }
	}
	printf("Done!\n");
